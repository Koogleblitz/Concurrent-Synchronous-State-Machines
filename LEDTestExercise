 ...
 *  Partner(s) Name: 
 *	Lab Section: 21
 *	Assignment: Lab #11  sample exercise
...
//------shared vars-------//
unsigned char led0Out = 0;
unsigned char led1Out = 0;
...
//---------------------------------------States----------------------------------------------//
enum pseBtnStates{pseBtnWait, pseBtnPress, pseBtnRelease };
enum led0States{led0Wait, led0Blink };
enum led1States{led1Wait, led1Blink };
...
int pseBtnTick(int state){	

	unsigned char press = ~PINA & 1;
	switch (state){
		case pseBtnWait:
			state = press == 1? pseBtnPress: pseBtnWait; break;
		case pseBtnPress:
			state = pseBtnRelease; break;
		case pseBtnRelease:
			state = press == 0? pseBtnWait: pseBtnPress;	break;
		default: state = pseBtnWait;	break;
	}
//--------------------------------------
	switch (state){
		case pseBtnWait: break;
		case pseBtnPress:
			pause = (pause == 0) ? 1: 0;  
			break;
		case pseBtnRelease:	break;
	}
	//PORTB = 8;
	return state;
}
...
int led0Tick(int state){
	switch (state){
		case led0Wait:	state = !pause? led0Blink: led0Wait; break;
		case led0Blink:	state = pause? led0Wait: led0Blink;	break;
		default: state = led0Wait;	break;
	}
	//---------------------------------
	switch (state){
		case led0Wait: break;
		case led0Blink: 
			led0Out = (led0Out == 0)? 1 : 0;	
			break;
	}
	//PORTB = 4;
	return state;
...
int led1Tick(int state){
	switch (state){
		case led1Wait:	state = !pause? led1Blink: led1Wait; break;
		case led1Blink:	state = pause? led1Wait: led1Blink;	break;
		default: state = led1Wait;	break;
	}
	//----------------------------------
	switch (state){
		case led1Wait: break;
		case led1Blink: 
			led1Out = (led1Out == 0)? 1 : 0;	
			break;
	}
	//PORTB =2;
	return state;
...
int main(void) {
		...
		//use the first task's period as the base case, find the GCD of that and the second task, then find the GCD of that GCD with the third and so on 			//until we end up with the common GCD of all tasks. 
